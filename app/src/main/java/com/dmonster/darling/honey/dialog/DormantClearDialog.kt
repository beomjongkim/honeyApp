...
class DormantClearDialog: androidx.fragment.app.DialogFragment() {
    ...
    private fun init() {
        ...
        dialog?.context?.let {
            ...
            tv_dlg_basic_content.text = it.resources.getString(R.string.msg_main_dormant_clear)
    ...
    private fun setListener() {
        ...
        disposeBag.add(RxView.clicks(tv_dlg_basic_enter)
                .throttleFirst(1, TimeUnit.SECONDS)
                .subscribe {
                    setDormant(id, AppKeyValue.instance.keyNo)
                })
    }
                ...
                item.let {
                    if(it.isSuccess) {
                        dialog?.context?.let { it1 ->
                            Utility.instance.showAlert(it1, it1.getString(R.string.app_name), it1.getString(R.string.msg_main_dormant_clear_complete), DialogInterface.OnClickListener { dialog, which ->
                                Utility.instance.UserData().setUserDormant(AppKeyValue.instance.keyNo)
                                val bundle = Bundle()
                                bundle.putString(FirebaseAnalytics.Param.ITEM_ID, id)
                                FirebaseAnalytics.getInstance(it1).logEvent("unSetDormant",bundle)
                                dismiss()
                            })
                        }
                    }
