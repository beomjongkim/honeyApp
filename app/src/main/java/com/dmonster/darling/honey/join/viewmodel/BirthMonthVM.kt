...
class BirthMonthVM : BaseEditRegexVM()  {
    ...
    override fun onTextChange(editText: EditText, s: CharSequence?, start: Int, before: Int, count: Int) {
        ...
        s?.let {
            if (!s.isNullOrBlank()) {
                if (it.contains(Regex(RegexStr.instance.overOneDigit)) && (it.toString().toInt() <= 12) && (it.toString().toInt() > 0)) {
                    mPassed. value =true
                } else {
                    editText.setText("")
                    Utility.instance.showToast(editText.context, "1~12 사이의 값을 입력해주세요")
                }
            }
        }
    }
