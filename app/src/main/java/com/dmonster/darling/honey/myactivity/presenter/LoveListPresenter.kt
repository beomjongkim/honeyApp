...
class LoveListPresenter: LoveListContract.Presenter {

    private lateinit var mModel: MyActModel
    private lateinit var mView: LoveListContract.View
    private lateinit var subscription: CompositeDisposable

    override fun attachView(view: LoveListContract.View) {
        this.mView = view
    ...
    /*    러브카드 목록    */
    override fun getLoveList(isScroll: Boolean, startCnt: String?, limitCnt: String?, id: String?) {
        val subscriber = object: DisposableObserver<ResultListItem<MemberData>>() {
            ...
            override fun onError(e: Throwable) {
                mView.setLoveListFailed(e.message)
                e.printStackTrace()
            }
                ...
                item.let { it ->
                    if(it.isSuccess) {
                        it.items?.let {
                            ...
                            mList.addAll(it)
                            mView.setLoveListComplete(isScroll, mList)
                        }
                    }
                    else {
                        mView.setLoveListFailed(it.message)
                    }
                }
            }
        }
        mModel.requestLoveList(startCnt, limitCnt, id, subscriber)
