    ...
    private fun setListener() {
        ...
        rv_frag_my_act_profile_list.addOnScrollListener(object: RecyclerView.OnScrollListener() {
            override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                ...
                srl_frag_my_act_profile_layout.isEnabled = cl_frag_my_act_profile_layout.getChildAt(0).top == 0
            }

            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                super.onScrolled(recyclerView, dx, dy)
                val lastVisibleItemPosition = (recyclerView.layoutManager as LinearLayoutManager).findLastCompletelyVisibleItemPosition().plus(1)
                val itemTotalCount = recyclerView.adapter?.itemCount
                if(lastVisibleItemPosition == itemTotalCount && itemTotalCount >= (AppKeyValue.instance.listLimitCnt.toInt())) {
                    mPresenter.getProfileList(true, lastVisibleItemPosition.toString(), AppKeyValue.instance.listLimitCnt, id, AppKeyValue.instance.profileListVisit)
                }
            }
        })
                    ...
                    if(mAdapter.data.size > 0) {
                        context?.let { it1 ->
                            mAdapter.checkArray?.size?.let { it2 ->
                                if(it2 > 0) {
                                    Utility.instance.showTwoButtonAlert(it1, it1.resources.getString(R.string.app_name), it1.resources.getString(R.string.msg_my_activity_profile_delete), DialogInterface.OnClickListener { dialog, which ->
                                        if(which == DialogInterface.BUTTON_POSITIVE) {
                                            ...
                                            mAdapter.checkArray?.indices?.let { it3 ->
                                                for(i in it3) {
                                                    ...
                                                    mPresenter.setProfileDelete(id, "visit", idx)
                                                }
                                            }
                                        }
                                    })
                                    ...
                                    Utility.instance.showToast(it1, it1.resources.getString(R.string.msg_my_activity_select_profile))
                                }
                            }
                        }
                    }
    ...
    private fun itemTalkClickListener() = View.OnClickListener {
        context?.let { it1 ->
            ...
            if(gender == otherGender) {
                Utility.instance.showAlert(it1, it1.resources.getString(R.string.app_name), it1.resources.getString(R.string.msg_profile_error_gender), DialogInterface.OnClickListener { dialog, which -> })
            }
            else {
                ll_frag_my_act_profile_progress.visibility = View.VISIBLE
    ...
    override fun setItemCheckComplete(result: String?) {
        context?.let {
            when(result) {
                ...
                "N" -> {
                    val content = String.format(it.resources.getString(R.string.msg_profile_item), it.resources.getString(R.string.msg_profile_item_talk))
                    Utility.instance.showTwoButtonAlert(it, it.resources.getString(R.string.app_name), content, DialogInterface.OnClickListener { dialog, which ->
                        if(which == DialogInterface.BUTTON_POSITIVE) {
